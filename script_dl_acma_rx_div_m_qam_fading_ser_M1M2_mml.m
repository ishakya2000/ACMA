...
function [simSer, Thpt, simSer_, Thpt_,opt_rot] = script_dl_acma_rx_div_m_qam_fading_ser_M1M2_mml(esno_1,esno_2,n,m1,m2,p1,p2,l,ch,rotate,mml)

% [simSer, Thpt, simSer_, Thpt_]= script_dl_acma_rx_div_m_qam_fading_ser_M1M2_mml(20,20,1e+4,16,16,0.9,0.1,1,1,0,1)
%clear
rp=[];
...
if rotate==100
    ...
    [SNR_loss_M12_full(q),sq_dist_col(q),sq_dist_M12_full(q)]=CSMA_minimum_dist_papr_analysis(M1,M2,p1,p2,(2*pi)/(Vlen-q+1),0);
...
MQAM1_const_non=MQAM1_const_mag.*exp(i*(MQAM1_const_ang));
...
for b=1:length(MQAM2_const)
    ...
    non_coll_MQAM=[MQAM1_const_non.' repmat(MQAM2_const(1,b),length(MQAM1_const),1)];
    non_colla_MQAM= [ non_colla_MQAM; non_coll_MQAM];
end
...
r___=[];
    ...
    if L==1        
        ...
        if mml
            d_poss = abs(r___(1:M1*M2,:)-(kron(h__(1,:),(sqrt(P1)*k*non_colla_MQAM(:,1)+sqrt(P2)*k_*non_colla_MQAM(:,2))))).^2;
        end
    ...
    d_poss = abs(r___(1:M1*M2,:)-(kron(h__(1,:),(sqrt(P1)*k*colla_MQAM(:,1)+sqrt(P2)*k_*colla_MQAM(:,2))))).^2 ...
         ...
         if mml
             ...
             d_poss = abs(r___(1:M1*M2,:)-(kron(h__(1,:),(sqrt(P1)*k*non_colla_MQAM(:,1)+sqrt(P2)*k_*non_colla_MQAM(:,2))))).^2 ...
             + abs(r___(M1*M2+1:M1*M2*L,:)-(kron(h__(2,:),(sqrt(P1)*k*non_colla_MQAM(:,1)+sqrt(P2)*k_*non_colla_MQAM(:,2))))).^2;
